...
if [ $DRUSH == "y" ]; then
  ...
  if [ -d "${dest}/profiles/projects" ]; then
    ...
    then
      printf "%s\n" $R"FAILED"$N
      printf "%s\n" $G"Want to install a simple installation with drush?"$N
      askcontinue
      drush dl drupal --drupal-project-rename=$NAME
    else
      printf "%s\n" $G"Profile created!"$N
      drush make profiles/project/build-project.make
      askcontinue
      drush si project --account-pass=admin --site-name="${NAME}" --db-url="mysql://${NAME}:${NAME}@localhost/${NAME}" --yes
    fi
  fi
fi
# summary output
